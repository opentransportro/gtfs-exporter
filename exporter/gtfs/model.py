...
class CalendarDate(object):
    ...
    def _coerce(self, other):
        ...
        if isinstance(other, CalendarDate):
            return other.date
        raise ValueError(
            "Can't coerce %s to a CalendarDate (use either a CalendarDate, a 'yyyy-mm-dd' string or a datetime.date)" % other)
    ...
    def __repr__(self):
        if hasattr(self, 'feed_id') and self.feed_id is not None and hasattr(self,
                                                                             'service_id') and self.service_id is not None:
            return "<%s(id=%s/%s %s)>" % (
