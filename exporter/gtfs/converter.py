...
from exporter.gtfs.model import Agency, FeedInfo, Route, Calendar, CalendarDate, Stop, \
    Trip, StopTime, Transfer, Shape, ShapePoint, Zone, FareAttribute, FareRule
from exporter.gtfs.spatial import DistanceCache, orthodromic_distance, \
    orthodromic_seg_distance
from exporter.gtfs.utils import timing, fmttime, ContinousPiecewiseLinearFunc
...
def _convert_gtfs_model(feed_id, gtfs, dao, lenient=False, disable_normalization=True):
    ...
    if disable_normalization:
        ...
        for shape in dao.shapes(fltr=Shape.feed_id == feed_id, prefetch_points=True, batch_size=50):
            ...
            for trip in dao.trips(fltr=(Trip.feed_id == feed_id) & (Trip.shape_id == shape.shape_id),
        ...
        for trip in dao.trips(fltr=(Trip.feed_id == feed_id) & (Trip.shape_id == None), prefetch_stop_times=True,
